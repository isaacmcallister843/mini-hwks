...
from .python.p_mv_exp import mv_exp_ll, mv_exp_fit_em, mv_exp_sample_branching, mv_exp_fit_em_fix_theta
...
class MultivariateExpHawkesProcess(PointProcess):
    ...
    def fit_const_theta(self, theta, t, c, T=None, **kwargs):

        t, c, T = self._prep_t_c_T(t, c, T)

        emkwargs = {k: v for k, v in kwargs.items() if k in ["maxiter", "reltol"]}

        ll, params, _ = mv_exp_fit_em_fix_theta(t, c, T, theta = theta,  **emkwargs)
        self.set_params(*params)
        return ll
